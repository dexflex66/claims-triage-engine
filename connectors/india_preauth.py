"""India pre-authorization connector with live HTTP transport + safe fallback."""
...
class IndiaPreauthConnector:
    """Connector for India pre-auth submission flow."""

    channel = "india_preauth"
    ...
    def __init__(self) -> None:
        self.contract = load_contract("india_preauth")
        self.base_url = os.environ.get("IN_PREAUTH_BASE_URL", "").rstrip("/")
        self.submit_path = os.environ.get("IN_PREAUTH_SUBMIT_PATH", "/api/v1/preauth/submit")
        self.status_path_template = os.environ.get("IN_PREAUTH_STATUS_PATH_TEMPLATE", "/api/v1/preauth/{external_ref}/status")
        self.receipt_path_template = os.environ.get("IN_PREAUTH_RECEIPT_PATH_TEMPLATE", "/api/v1/preauth/{external_ref}/receipt")
        self.api_token = os.environ.get("IN_PREAUTH_API_TOKEN", "")
        self.hmac_secret = os.environ.get("IN_PREAUTH_HMAC_SECRET", "")
        self.hmac_key_id = os.environ.get("IN_PREAUTH_HMAC_KEY_ID", "")
        self.timeout_seconds = _safe_float(os.environ.get("IN_PREAUTH_TIMEOUT_SECONDS", "10"), 10.0)
        self.max_retries = _safe_int(os.environ.get("IN_PREAUTH_MAX_RETRIES", "3"), 3)
        self.backoff_seconds = _safe_float(os.environ.get("IN_PREAUTH_BACKOFF_SECONDS", "0.5"), 0.5)
        self.enforce_contract = os.environ.get("IN_PREAUTH_ENFORCE_CONTRACT", "true").lower() == "true"
    ...
    def _request_json(
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                ...
                raise RuntimeError(f"IN_PREAUTH HTTPError code={exc.code} detail={detail}") from exc
            except URLError as exc:
                last_err = exc
                if attempt < self.max_retries:
                    time.sleep(self.backoff_seconds * attempt)
                    continue
                raise RuntimeError(f"IN_PREAUTH URLError: {exc}") from exc

        raise RuntimeError(f"IN_PREAUTH request failed after retries: {last_err}")
    ...
    def submit(self, packet, idempotency_key: str) -> SubmissionAck:
        ...
        submission_id = str(response.get("submission_id") or f"sub_{uuid4().hex[:16]}")
        external_ref = str(response.get("external_ref") or f"inpre_{uuid4().hex[:12]}")
        status = self._normalize_status(str(response.get("status") or "submitted"))
        return SubmissionAck(
    ...
    def poll_status(self, external_ref: str) -> StatusUpdate:
        ...
        status = self._normalize_status(str(response.get("status") or "processing"))
        return StatusUpdate(
            external_ref=external_ref,
            status=status,
    ...
    def fetch_receipt(self, external_ref: str) -> ReceiptArtifact:
        ...
        artifact_ref = str(response.get("artifact_ref") or f"receipt://{self.channel}/{external_ref}")
        return ReceiptArtifact(
            external_ref=external_ref,
